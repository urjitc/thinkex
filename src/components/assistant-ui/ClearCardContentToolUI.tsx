 ...
 */
const ClearCardContentInner: FC<{
  result: any;
  ...
  if (status.type === "complete" && parsed?.success) {
    content = (
      <div className="mb-4 flex items-center gap-2 rounded-lg border border-green-200 bg-green-50 px-4 py-3 dark:border-green-800 dark:bg-green-950">
        ...
        <p className="text-sm text-green-800 dark:text-green-200">Card content cleared successfully</p>
      </div>
    );
...
ClearCardContentInner.displayName = "ClearCardContentInner";
...
export const ClearCardContentToolUI = makeAssistantToolUI({
  ...
  render: function ClearCardContentUI({ result, status }) {
    ...
    return (
      <ToolUIErrorBoundary componentName="ClearCardContent">
        <ClearCardContentInner result={result} status={status} />
      </ToolUIErrorBoundary>
    );
  },
});
