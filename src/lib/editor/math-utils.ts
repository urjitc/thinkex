/**
 * Math utility functions for BlockNote editor
 * Used for both server-side markdown conversion and client-side paste handling
 */

// Block type placeholder (works with any block structure)
type AnyBlock = any;
 ...
 */
export function convertMathInBlocks(blocks: AnyBlock[]): AnyBlock[] {
 ...
 */
function processBlockForMath(block: AnyBlock): AnyBlock | AnyBlock[] {
 ...
 */
function processChildBlocks(blocks: AnyBlock[]): AnyBlock[] {
    const result: AnyBlock[] = [];
    for (const block of blocks) {
        const processed = processBlockForMath(block);
        if (Array.isArray(processed)) {
